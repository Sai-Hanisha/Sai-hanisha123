    ...
    if(is_number(Id) and name.isalpha()):
        ...
        while(True):
            ...
            for (x,y,w,h) in faces:
                ...
                cv2.imwrite("TrainingImage\ "+name +"."+Id +'.'+ str(sampleNum) + ".jpg", gray[y:y+h,x:x+w])  #saving the captured face in the dataset folder TrainingImage
                cv2.imshow('frame',img)      #display the frame
            
            if cv2.waitKey(10) & 0xFF == ord('q'):    #wait for 100 miliseconds 
                break
...
copyWrite.insert("insert", "Developed by Hanisha","", "TEAM", "superscript")
